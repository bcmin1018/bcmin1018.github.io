...
하나의 잡(job)이 **20개가 넘는 외부 API 호출로 쪼개지는** 다화자 음성 합성(TTS) 파이프라인이었다. 대본을 대사 단위 세그먼트로 나눠 각 세그먼트를 해당 화자의 보이스로 합성 요청하고, 응답 파일이 **전부 모여야** 원래 대사 순서대로 병합해 결과물을 만들 수 있다. 이 요구사항은 세 가지 어려움을 동반한다.
...
처리 흐름은 이렇다: 잡 안의 세그먼트를 모두 요청하고 응답을 기다린다 → 실패한 세그먼트는 재시도한다 → 그래도 실패하면 Failed로 기록한다.

{% include figure image_path="/assets/images/posts/2026-07-19-tts-pipeline.svg" alt="다화자 TTS 파이프라인: 대본을 대사 단위 세그먼트로 나눠 화자별 보이스로 TTS를 요청하고, 모든 조각이 도착하면 원래 순서로 병합한다" caption="대본 → 화자별 fan-out → 조각 수집 → 원래 순서로 병합. 화자별로 나눠 요청하지만 결과는 대사 순서 그대로여야 한다." %}

이 흐름을 안전하게 돌리기 위해 필요했던 것이 아래 6가지다.
